...
const CAT_STYLE: Record<string, { bg: string; iconColor: string }> = {
  "Electronics":    { bg: "cat-bg-electronics", iconColor: "#3b82f6" },
  "Fashion":        { bg: "cat-bg-fashion",      iconColor: "#ec4899" },
  "Home & Kitchen": { bg: "cat-bg-home",         iconColor: "#10b981" },
  "Books":          { bg: "cat-bg-books",         iconColor: "#d97706" },
  "Sports":         { bg: "cat-bg-sports",        iconColor: "#ef4444" },
};
...
export function CategoryCard({ cat, count, iconName, index = 0 }: CategoryCardProps) {
  const Icon = ICON_MAP[iconName] ?? Package;
  const style = CAT_STYLE[cat] ?? { bg: "cat-bg-default", iconColor: "#6366f1" };
