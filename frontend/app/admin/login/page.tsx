...
import { ShoppingBag, Loader2, ArrowLeft, ShieldCheck, Eye, EyeOff } from "lucide-react";
...
export default function AdminLoginPage() {
  ...
  const { register, handleSubmit, setValue, formState: { errors, isSubmitting } } = useForm<FormValues>({
    ...
    mode: "onTouched",
    // Pre-fill credentials so admin can log in with one click
    defaultValues: { email: ADMIN_EMAIL, password: ADMIN_PASSWORD },
  });

  // If already authenticated as admin, skip login page entirely
  useEffect(() => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Hard redirect ensures fresh mernshop_admin cookie reaches middleware
    ...
    <div className="flex min-h-screen items-center justify-center bg-parchment px-4 dark:bg-dark-bg">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-border bg-white p-8 shadow-sm dark:border-dark-border dark:bg-dark-surface">
          ...
          <form onSubmit={(e) => { void handleSubmit(onSubmit)(e); }} className="flex flex-col gap-4" noValidate autoComplete="off">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  {...register("password")}
                  className="h-11 w-full rounded-lg border border-border bg-white px-3 pr-11 text-[14px] text-ink outline-none transition-all placeholder:text-ink-muted/50 focus:border-amber focus:ring-1 focus:ring-amber/30 dark:border-dark-border dark:bg-dark-surface-2 dark:text-white"
                />
                <button
                  type="button"
            ...
            <div className="flex items-center gap-2 rounded-lg border border-border bg-cream px-3 py-2.5 dark:border-dark-border dark:bg-dark-surface-2">
              ...
              <p className="text-xs text-ink-muted">
                Credentials are pre-filled for demo. Click Sign In.
              </p>
            </div>
